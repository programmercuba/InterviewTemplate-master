    ...
    public static class DbInitializer
    {
        public static void Initialize(FootballContext context)
        {
            ...
            var matches = new Match[]
            {
                new Match
                {
                    HouseManager = managers.FirstOrDefault(),
                    AwayManager = managers.LastOrDefault(),
                    HousePlayers = players.ToList(),
                    AwayPlayers = players.ToList(),
                    Referee = referees.FirstOrDefault(),
                    Date = System.DateTime.Now
                }
            };

            foreach (var mt in matches)
                context.Matches.Add(mt);
            context.SaveChanges();
        }
    }
}
